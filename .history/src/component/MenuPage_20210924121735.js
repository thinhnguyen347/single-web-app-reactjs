...
export default function MenuPage() {
  ...
  function addToList(id) {
    ...
    if (list[index].amount > 0) {
      ...
      let cart_current = localStorage.setItem("cart");
      if (cart_current.length > 0){
        localStorage.setItem("cart", localStorage.setItem("cart", JSON.stringify(addedItems.current))JSON.stringify(addedItems.current))
      } else{
