...
export default function ShippingInfo({ moveToPaymentMethod, returnToCart }) {
  ...
  const [discount, setCode] = useState(0);
  ...
  let total_temp= addedList.map(item=> item.price * item.amount)
          ...
          <div className="row g-4">
            <div className="col-12 col-md-5 py-0 ps-md-0 border-end border-dark">
              ...
              <div className="container-fluid mx-0 px-0 d-flex justify-content-between align-items-center">
                <label htmlFor="code" className="d-block me-5">Mã giảm giá:</label>
                <input id="code" className="d-block p-2 flex-fill ms-5" placeholder="Nhập mã giảm giá..."/>
              </div>
              <hr />
              <div className="container-fluid mx-0 px-0 d-flex justify-content-between">
                <span className="d-block">Tạm tính:</span>
                <span className="d-block">100k</span>
              </div>
              <div className="container-fluid mx-0 px-0 d-flex justify-content-between">
