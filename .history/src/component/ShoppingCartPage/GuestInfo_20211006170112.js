...
export default function GuestInfo({ returnToCart, goToPaymentInfo }) {
  ...
  return (<>
    ...
    <div
    className={`alert alert-danger text-center position-fixed start-50 translate-middle mb-0 ${
      alertFail ? "bottom-100 opacity-100" : "top-100 opacity-100"
    }`}
    role="alert"
  >
    Quý khách vui lòng nhập hoặc kiểm tra lại thông tin cá&nbsp;nhân &amp; phương thức giao hàng!
  </div></>
  );
}
