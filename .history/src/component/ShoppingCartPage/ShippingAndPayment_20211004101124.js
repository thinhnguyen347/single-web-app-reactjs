...
export default function ShippingAndPayment({ returnToCart }) {
  ...
  const cartQuantity = useSelector((state) => state.shippingFee.quantity);
  ...
  let shippingFee = subtotals > 300000 ? 0 : 25000;
    ...
    <>
      <nav aria-label="breadcrumb" className="ms-4 ms-lg-5 py-3">
        <ol className="breadcrumb">
          ...
          <li className={`breadcrumb-item ${!paymentOn && "text-primary active"}`}>
            Thông tin giao hàng
          </li>
          <li className={`breadcrumb-item ${paymentOn && "text-primary active"}`}>Thanh toán</li>
        </ol>
      </nav>
      <section className="px-3 py-5 mb-5 p-md-3">
        <h2 className="text-center">{paymentOn ? "Thanh toán": "Thông tin giao hàng"}</h2>
          ...
          <div className="row g-4">
            <div className="col-12 col-md-6 py-0 border-end border-dark">
              {paymentOn ? (
                ...
                <GuestInfo returnToCart={returnToCart} goToPaymentInfo={goToPaymentInfo} />
              )}
            </div>
