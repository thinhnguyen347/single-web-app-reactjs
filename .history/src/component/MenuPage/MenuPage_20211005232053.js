...
export default function MenuPage() {
  ...
  function addToList(id) {
    ...
    //console.log("cart_current", cart_current); localStorage.setItem("cart","[]")
    if (cart_current.length !== 0) {
      indexCart < 0 && index1 < 0
        ? addedItems.current.push(...cart_current)
        : (addedItems.current[index].amount += cart_current[indexCart].amount);
    }

    console.log("114", addedItems.current);
    ...
    if (addedItems.current.length === 0 && index1 < 0) {
      ...
      setAlertSuccess(true);
      timer.current = setTimeout(() => setAlertSuccess(false), 2000);
      console.log("case 3", addedItems.current[index1].amount, list[index].amount);
    }
