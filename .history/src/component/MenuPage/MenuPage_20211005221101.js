...
export default function MenuPage() {
  ...
  function addToList(id) {
    ...
    let indexCart = cart_current.findIndex((item) => item.id === id);

    console.log(addedItems.current);
    console.log("cart_current", cart_current);
    if (cart_current.length !== 0) {
      if (indexCart < 0)
        cart_current.map((item, index) => addedItems.current.push(item[index]));
      if (indexCart > 0 && addedItems.current.length > 0)
        addedItems.current[index].amount += cart_current.amount;
    }
    ...
    console.log("addedItems", addedItems.current);

    let index1 =
      addedItems.current.length > 0
        ? addedItems.current.findIndex((item) => item.id === id)
        : -1;

    if (addedItems.current.length === 0) {
      addedItems.current.push(list[index]);
      setAlertSuccess(true);
      timer.current = setTimeout(() => setAlertSuccess(false), 2000);
      console.log("case 1");
    } else if (addedItems.current.length > 0) {
      if (index1 < 0) {
        addedItems.current.push(list[index]);
        console.log("case 2");
      } else {
        addedItems.current[index1].amount += list[index].amount;
        console.log("case 3");
      }
