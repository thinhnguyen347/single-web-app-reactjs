...
export default function MenuPage() {
  ...
  function addToList(id) {
    ...
    if (cart_current.length > 0) {
      cart_current.forEach((e, index)=> {
        let index1 = addedItems.current.findIndex((item) => item.id === e.id); //index_Of_CartItem_In_AddedList
        if (index1<0) addedItems.current.push(cart_current[indexInCart]);
      })
      addedItems.current.push(cart_current[indexInCart]);
      console.log('line120', cart_current[indexInCart])
    }

    if (cart_current.length > 0 && indexInAddedList < 0) {
      addedItems.current.push(cart_current[indexInCart]);
      console.log('line120', cart_current[indexInCart])
    }
    ...
    let temp = [...list]; temp[index].amount=0;
    console.log(temp)
    //setProducts(temp);
  }
