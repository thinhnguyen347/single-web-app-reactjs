...
export default function MenuPage() {
  ...
  function addToList(id) {
    ...
    let cart_current = JSON.parselocalStorage.getItem("cart");
    console.log(index, index1, cart_current)
    console.log(list[index].amount);


    if (cart_current.length === 0 && index1 < 0 && list[index].amount > 0) {
      addedItems.current.push(list[index]);
      setAlertSuccess(true);
      timer.current = setTimeout(() => setAlertSuccess(false), 2000);
      console.log("case 1")
    } else if (
      cart_current.length > 0 &&
      index1 >= 0 &&
      list[index].amount > 0
    ) {
      let cart_obj = JSON.parse(cart_current);
      cart_obj.map((item) => addedItems.current.push(item));
      index1 = addedItems.current.findIndex((item) => item.id === id);
      addedItems.current[index1].amount += list[index].amount;
      setAlertSuccess(true);
      timer.current = setTimeout(() => setAlertSuccess(false), 2000);
      console.log("case 2", addedItems.current)
      ...
      localStorage.setItem("cart", JSON.stringify(addedItems.current)); console.log(localStorage)
    setProducts([...originList]);
  }
