...
export default function MenuPage() {
  ...
  function addToList(id) {
    ...
    if (cart_current.length !== 0) {
      if (indexCart < 0) cart_current.map((item, index) => addedItems.current.push(item[index]));
      if (indexCart > 0 && addedItems.current.length > 0) addedItems.current[index].amount += cart_current.amount;
    }
    ...
    if (addedItems.current.length === 0 || addedItems.current.length > 0 && index1 < 0) {
...
if(index1 < 0) addedItems.current.push(list[index]);
    ...
    if (index1 >= 0 && addedItems.current.length > 0) {
