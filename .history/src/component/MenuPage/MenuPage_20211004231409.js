...
export default function MenuPage() {
  ...
  function addToList(id) {
    ...
    if (cart_current.length === 0 && index1 < 0) {
    ...
    if (list[index].amount > 0) {
      if (index1 >= 0) {
        addedItems.current[index1].amount = 0;
        //setExistItem(true);
        timer.current = setTimeout(() => setExistItem(false), 2000);
      }
  ...
  return (
    <>
      ...
      <div
        className={`alert alert-warning position-fixed bottom-0 end-0 mx-3 mx-md-5 ${
          existItem && "opacity-100"
        }`}
        role="alert"
      >
        Sản phẩm đã có trong giỏ. Vui lòng điều chỉnh số lượng trong giỏ hàng!
      </div>
