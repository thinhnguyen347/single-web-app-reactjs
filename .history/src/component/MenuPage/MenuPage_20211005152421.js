...
export default function MenuPage() {
  ...
  function addToList(id) {
    ...
    if (cart_current.length > 0 && index1 < 0 && list[index].amount > 0) {
      let index2 = cart_current.findIndex((item) => item.id === id);
      addedItems.current.push(cart_current[index2]);
      setAlertSuccess(true);
      timer.current = setTimeout(() => setAlertSuccess(false), 2000);
      console.log("case 1");
    }
    ...
    if (cart_current.length > 0 && index1 >= 0 && list[index].amount > 0) {
      ...
      let index2 = addedItems.current.findIndex((item) => item.id === id);
      addedItems.current[index2].amount += list[index].amount;
      setAlertSuccess(true);
      timer.current = setTimeout(() => setAlertSuccess(false), 2000);
      console.log("case 3");
    } 
