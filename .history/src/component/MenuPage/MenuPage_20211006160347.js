...
export default function MenuPage() {
  ...
  let timer = useRef(), searchResult = useRef();
  ...
  function addToList(id) {
    ...
    let addedItems = [];
    addedItems.push(list[index]); // localStorage.setItem("cart","[]")
    ...
    addedItems.current.forEach((element) => {
      let index1 = cart_current.findIndex((item) => item.id === element.id); //index_Of_CartItem_In_AddedList
      if (index1 < 0) {
        cart_current.push(element);
      } else {
        cart_current[index1].amount += element.amount;
      }
      //console.log(`cart item amount: ${cart_current[index1].amount} + ${element.amount}`)
    });
    ...
    temp[index].amount = 0;
    console.log("151 cart", cart_current);
    setProducts(temp);
  }
