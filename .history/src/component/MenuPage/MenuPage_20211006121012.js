...
export default function MenuPage() {
  ...
  function addToList(id) {
    ...
    let indexInAddedList = addedItems.current.findIndex((item) => item.id === id);

    console.log("115cart_current", cart_current, indexCart); // localStorage.setItem("cart","[]")
    if (cart_current.length !== 0 && indexCart < 0 && indexInAddedList < 0)
    ...
    if (addedItems.current.length === 0 && indexInAddedList < 0) {
      ...
      console.log("case 1");
    } else if (addedItems.current.length > 0 && indexInAddedList < 0) {
      addedItems.current.push(list[index]);
      setAlertSuccess(true);
      timer.current = setTimeout(() => setAlertSuccess(false), 2000);
      console.log("case 2");
