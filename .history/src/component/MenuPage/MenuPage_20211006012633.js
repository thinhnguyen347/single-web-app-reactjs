...
export default function MenuPage() {
  ...
  const cloneOriginList = originList.map((item) => ({ ...item }));
  ...
  function addToList(id) {
    ...
    console.log("cart_current", cart_current, indexCart); // localStorage.setItem("cart","[]")
    //if (cart_current.length !== 0 && indexCart < 0 && index1 < 0) addedItems.current.push(...cart_current)
    ...
    if (addedItems.current.length === 0 && index1 < 0) {
      ...
      console.log(
        "case 3",
        addedItems.current[index1].amount,
        list[index].amount
      );
    }

    localStorage.setItem("cart", JSON.stringify(addedItems.current));
    setProducts(cloneOriginList);
    console.log(cloneOriginList)
  }
