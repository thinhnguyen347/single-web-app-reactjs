...
export default function MenuPage() {
  ...
  function addToList(id) {
    ...
    let indexIn = list.findIndex((item) => item.id === id);
    ...
    if (addedItems.current.length === 0 && indexInAddedList < 0) {
      addedItems.current.push(list[indexIn]);
      ...
      addedItems.current[indexInAddedList].amount += list[indexIn].amount;
      setAlertSuccess(true);
      timer.current = setTimeout(() => setAlertSuccess(false), 2000);
      console.log(
        "case 3",
