...
export default function MenuPage() {
  ...
  function addToList(id) {
    ...
    if (cart_current.length > 0) {
      cart_current.forEach((element, index) => {
        let index1 = addedItems.current.findIndex((item) => item.id === element.id); //index_Of_CartItem_In_AddedList
        if (index1 < 0) {
          addedItems.current.push(element);
        } else {
          addedItems.current[index1].amount += element.amount;
        }
      });
