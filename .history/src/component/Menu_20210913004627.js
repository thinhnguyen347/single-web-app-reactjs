...
function MainContent({ img, title, price,amount }) {
  return (
    <div className="card h-100">
      ...
      <div className="card-body d-flex flex-column">
        ...
        <div className="spinbox d-flex flex-row mx-auto">
          ...
          <div className="d-flex w-100 align-items-center justify-content-center border-top border-bottom border-secondary" onClick={(e)=> e.stopPropagation()}>
      ...
      <section className="px-3 py-5 p-md-5">
        ...
        <div className="container-fluid pt-4">
          <div className="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-4 g-lg-2">
            {temp.map(({ id, img, title, price, slug,amount }) => (
              <Link
                to={`/menu/${slug}`}
                key={id}
                className="col col-md-6 col-lg-3 col-lg-2 p-1 p-md-2 p-lg-3 text-decoration-none text-dark"
              >
                <MainContent img={img} title={title} price={price} amount={amount} increase={increase} decrease={e.stopPropagation();} />
              </Link>
            ))}
          </div>
        </div>
