...
export default function NavBar() {
  ...
  useEffect(() => {
    fetch(`https://yummy-db.herokuapp.com/products?q=${input}&attr=title`)
      .then((res) => res.json())
      .then((result) => {
              ...
              {searchResult.length > 0 && (
                <div className="result-modal container position-absolute top-100 start-0 bg-light rounded">
                  ...
                  {searchResult.map(({ img, title, id, slug }) => (
                    <Link to={`menu/${slug}/${id}`} key={id}>
                      <div className="row row-cols-2 pt-2 px-2 overflow-hidden" onClick={hideModal}>
