...
export default function MenuPage() {
  ...
  const [hidePageBtn, setHidePageBtn] = useState(null);
  ...
  useEffect(() => {
    if (isSearching)
      fetch(
        `${url}/products?title_like=${keyword}&_page=${currentPage}&_limit=4`
      )
        .then((res) => res.json())
        .then((result) => {
          setProducts(result);
          setIsLoading(false);
          if (result.length < 4)
            setHidePageBtn((currentPage) => currentPage + 1);
        });
            ...
            <ul className="pagination d-flex justify-content-center">
              <li className="page-item">
                <button
                  ...
                  aria-label="Previous"
                  disabled={currentPage === 1 || hidePageBtn}
                  onClick={() => {
                    if (currentPage > 1) {
