...
export default function MenuPage() {
  ...
  let list = [...products];
  //let originalList = products.map((item) => ({ ...item }));
  //originalList.find((item) => (item.id = item.id + "new"));

  useEffect(() => {
    fetch(`https://yummy-db.herokuapp.com/products?_page=${currentPage}&_limit=4`)
      .then((res) => res.json())
      .then((result) => {
        setProducts(result);
        setIsLoading(false);
  ...
  <ul className="pagination">
    <li className="page-item">
      <p className="page-link text-dark" aria-label="Previous">
        <span aria-hidden="true">&laquo;</span>
      </p>
    </li>
    <li className="page-item"><p className="page-link text-dark">1</p></li>
    <li className="page-item"><p className="page-link text-dark">2</p></li>
    <li className="page-item"><p className="page-link text-dark">3</p></li>
    <li className="page-item">
      <p className="page-link text-dark" aria-label="Next">
