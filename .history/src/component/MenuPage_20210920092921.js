...
export default function MenuPage() {
  ...
  const [originList, setCurrentPage] = useState(1);
  ...
  let newList = useRef();
  let cart = [...products];
  newList.current = list.map((item) => ({ ...item }));
  console.log(newList);
  ...
  function increase(id) {
    ...
    setProducts(list);
    console.log(index, list[index].amount);
  }
  ...
  function sort(e) {
    ...
    if (status === "0") {
      setProducts(newList.current);
    }
  }

  function addToList(id) {
    let index2 = cart.findIndex((item) => item.id === id);
    if (list[index2].amount > 0) {
      let index1 = addedItems.current.findIndex((item) => item.id === id);
      if (index1 >= 0) addedItems.current[index1].amount += cart[index2].amount;
      if (index1 < 0) addedItems.current.push(cart[index2]);
    } else {
      clearTimeout(timer.current);
      setAlert(true);
      timer.current = setTimeout(() => setAlert(false), 1500);
    }
