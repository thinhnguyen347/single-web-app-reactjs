...
export default function NavBar() {
  ...
  useEffect(() => {
    fetch(`https://yummy-db.herokuapp.com/products/title?q=${input}`)
      .then((res) => res.json())
      .then((result) => {
                ...
                <div className="container position-absolute top-100 start-0">
                  {searchResult.map(({img, title, id, slug}) => (
                    <div className="row row-cols-2 pt-2 px2 rounded bg-light overflow-hidden ">
                      ...
                      <img src={img} alt={slug} className="img-fluid rounded-circle"/>
                      </div>
                      <div className="col-8 mb-2 align-self-center">{title}</div>
                    </div>
                  ))}
                </div>
              ) : (
