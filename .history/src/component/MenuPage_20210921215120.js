...
export default function MenuPage() {
  ...
  let addedItems = useRef([...products]);
  let timer = useRef();
  ...
  useEffect(() => {
    if(isSearching) fetch(`${url}/products?_page=${currentPage}&_limit=4`)
      .then((res) => res.json())
      .then((result) => {
        setProducts(result);
        setIsLoading(false);
  ...
  useEffect(() => {
    fetch(`${url}/products?title_like=${keyword}_page=${currentPage}&_limit=4`)
      .then((res) => res.json())
      .then((result) => {
        setProducts(result);
        setIsLoading(false);
